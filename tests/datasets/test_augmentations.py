...
class TestAlbumentationsWrapperFromConfig:
    ...
    def test_native_target_aware_transforms_are_geometric(self):
        """Native multi-image transforms must be registered as target-affecting."""
        assert "CopyPaste" in GEOMETRIC_TRANSFORMS
        assert "MixUp" in GEOMETRIC_TRANSFORMS
...
class TestMosaicCompatibility:
    """Tests for Albumentations Mosaic compatibility with RF-DETR multi-image providers."""

    def test_from_config_instantiates_albumentations_mosaic(self):
        """Mosaic should be available through RF-DETR aug_config when Albumentations provides it."""
        transforms = AlbumentationsWrapper.from_config(
            {"Mosaic": {"grid_yx": (2, 2), "target_size": (64, 64), "cell_shape": (40, 40), "p": 1.0}}
        )

        assert len(transforms) == 1
        assert transforms[0].transform.transforms[0].__class__.__name__ == "Mosaic"

    def test_mosaic_uses_additional_sample_provider_and_keeps_fields_aligned(self):
        """Mosaic should combine provider samples and preserve per-instance tensor fields."""
        wrapper = AlbumentationsWrapper(alb.Mosaic(grid_yx=(2, 2), target_size=(64, 64), cell_shape=(40, 40), p=1.0))
        base_image = Image.fromarray(np.zeros((64, 64, 3), dtype=np.uint8))
        base_target = {
        ...
        for index in range(3):
            image = Image.fromarray(np.full((64, 64, 3), 50 * (index + 1), dtype=np.uint8))
            target = {
                "boxes": torch.tensor([[8.0, 8.0, 24.0, 24.0]], dtype=torch.float32),
                "labels": torch.tensor([index + 2], dtype=torch.long),
                "area": torch.tensor([256.0], dtype=torch.float32),
                "iscrowd": torch.tensor([index + 1], dtype=torch.long),
            }
            samples.append((image, target))
        cursor = {"index": 0}

        def provider():
            sample = samples[cursor["index"] % len(samples)]
            cursor["index"] += 1
            return sample

        wrapper.set_additional_sample_provider(provider)

        mosaic_image, mosaic_target = wrapper(base_image, base_target)

        assert mosaic_image.size == (64, 64)
        assert mosaic_target["boxes"].shape[0] == mosaic_target["labels"].shape[0]
        assert set(mosaic_target["labels"].tolist()) == {1, 2, 3, 4}
        assert mosaic_target["iscrowd"].shape[0] == mosaic_target["labels"].shape[0]
        assert mosaic_target["area"].shape[0] == mosaic_target["labels"].shape[0]
